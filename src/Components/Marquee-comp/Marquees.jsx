...
const Marquees = () => {
  ...
  return (
    isLargeScreen && (
      <div className="Marquee py-32 flex flex-col gap-12 text-white mt-10 overflow-hidden relative">
        {Data.map((imgUrl, index) => (
          <Marquee key={index} imgUrl={imgUrl} reverse={index % 2 === 1} />
        ))}

        <div className="ShadowWrap w-full h-full flex justify-center items-center pointer-events-none absolute top-0 left-0">
          <div className="Wrap w-full h-full bg-radial-vignette pointer-events-none"></div>
        </div>
      </div>
    )
  );
